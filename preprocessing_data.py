...
class Preprocessor():
    def __init__(self, tokenizer,gen_model_name,index_file_path):
    ...
    def prepare_index(self, documents, article_type):
        if os.path.exists(os.path.join(f'{self.gen_model_name.split("/")[1]}',f'{self.gen_model_name.split("/")[1]}_' + self.index_file_path.split('.')[0] + f'_{article_type}' + '.pkl')):
            start_time = time.time()
            with open(os.path.join(f'{self.gen_model_name.split("/")[1]}',f'{self.gen_model_name.split("/")[1]}_' + self.index_file_path.split('.')[0] + f'_{article_type}' + '.pkl'), "rb") as f:
                data = pickle.load(f)
            index = data["index"]
            ...
            tokenized_docs = [self.tokenizer(doc.text)['input_ids'] for doc in documents]
            end_time_tokenizing = time.time()
            print(f"Tokenized docs in time: {end_time_tokenizing - start_time_tokenizing:.4f} seconds")
            
            with open(os.path.join(f'{self.gen_model_name.split("/")[1]}',f'{self.gen_model_name.split("/")[1]}_' + self.index_file_path.split('.')[0] + f'_{article_type}' + '.pkl'), "wb") as f:
